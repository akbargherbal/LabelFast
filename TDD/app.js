...
function initializeApp() {
  ...
  function renderCorrectionArea() {
    ...
    if (window.correctedWords.length === 0 && !window.isEditingWord) { // Use window globals
  ...
  function selectWord(index) {
    ...
    else if (index < 0 && window.correctedWords.length > 0) window.selectedWordIndex = 0;
    else if (window.correctedWords.length > 0)
      window.selectedWordIndex = window.correctedWords.length - 1;
    else window.selectedWordIndex = -1;
    
    renderCorrectionArea(); // Calls the (now global) renderCorrectionArea
    
    const selectedSpan = window.correctionAreaEl.querySelector( // Use window.correctionAreaEl
      `span[data-index='${window.selectedWordIndex}']`
    );
    if (selectedSpan)
      selectedSpan.scrollIntoView({ // This will use the mock in tests
        behavior: "smooth",
        block: "nearest",
        inline: "nearest",
      });
  }
  window.selectWord = selectWord; // Expose
  ...
  function initiateWordEdit() {
    ...
    window.activeEditableElement.addEventListener("blur", () => {
      setTimeout(() => {
        if (window.isEditingWord && window.activeEditableElement) commitWordEdit();
      }, 0);
    });
  ...
  function loadSentence(index) {
    ...
    if (index >= window.sentencePairs.length) { // Use window.sentencePairs
  ...
  function globalKeyHandler(e) {
    ...
    if (isHelpModalOpen) { // Local variable
...
if (typeof window !== 'undefined' && typeof window.initializeApp === 'function' && document.readyState !== 'loading') {
  // If document is already loaded, call it directly
  window.initializeApp();
} else if (typeof window !== 'undefined' && typeof window.initializeApp === 'function') {
